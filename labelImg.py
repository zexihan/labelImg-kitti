...
from libs.kitti_io import KittiReader
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        save_format = action('&KITTI', self.change_format,
                      'Ctrl+', 'format_kitti', getStr('changeSaveFormat'), enabled=True)
    ...
    def set_format(self, save_format):
        if save_format == FORMAT_KITTI:
            self.actions.save_format.setText(FORMAT_KITTI)
            self.actions.save_format.setIcon(newIcon("format_kitti"))
            self.usingKittiFormat = True
            self.usingPascalVocFormat = False
            self.usingYoloFormat = False
    ...
    def change_format(self):
        if self.usingKittiFormat: self.set_format(FORMAT_PASCALVOC)
        elif self.usingPascalVocFormat: self.set_format(FORMAT_YOLO)
        elif self.usingYoloFormat: self.set_format(FORMAT_KITTI)
    ...
    def saveLabels(self, annotationFilePath):
        ...
        try:
            if self.usingKittiFormat is True:
                if annotationFilePath[-4:].lower() != ".txt":
                    annotationFilePath += TXT_EXT
                self.labelFile.saveKittiFormat(annotationFilePath, shapes, self.filePath, self.imageData, self.labelHist,
    ...
    def loadKITTITXTByFilename(self, txtPath):
        if self.filePath is None:
            return
